        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Button size="lg" asChild className="gap-2 px-8">
            <Link href={ROUTES.REGISTER}>
              {t("heroStartAnalyzing")}
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
          <Button size="lg" variant="outline" asChild className="gap-2 px-8 bg-transparent">
            <a href="#how-it-works">{t("heroLearnMore")}</a>
          </Button>
        </div>

        <div className="mt-16 grid grid-cols-3 gap-8">
          {[
            { value: "67.69%", label: t("heroStatAccuracy") },
            { value: "2", label: t("heroStatLanguages") },
            { value: "<2s", label: t("heroStatTime") },
          ].map((stat) => (
            <div key={stat.label}>
