...
export function TextInputForm({
  ...
  function handleTextSubmit(e: React.FormEvent) {
    ...
    if (text.length < MIN_TEXT_LENGTH) {
      toast.error(t.analyze.errorMinLength.replace("{min}", String(MIN_TEXT_LENGTH)));
      return;
    }
    if (text.length > MAX_TEXT_LENGTH) {
      toast.error(t.analyze.errorMaxLength.replace("{max}", String(MAX_TEXT_LENGTH)));
      return;
    }
  ...
  function handleFileSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedFile) {
      toast.error(t.analyze.errorSelectFile);
      return;
    }
  ...
  function validateAndSetFile(file: File) {
    if (file.size > MAX_FILE_SIZE_BYTES) {
      toast.error(t.analyze.errorFileSize.replace("{size}", String(MAX_FILE_SIZE_MB)));
      return;
    }
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="w-40">
          ...
          <Select value={language} onValueChange={setLanguage}>
            ...
            <SelectContent>
              <SelectItem value="auto">{t.analyze.languageAutoDetect}</SelectItem>
              <SelectItem value="ru">{t.analyze.languageRu}</SelectItem>
              <SelectItem value="kk">{t.analyze.languageKk}</SelectItem>
              <SelectItem value="en">{t.analyze.languageEn}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
          ...
          <form onSubmit={handleTextSubmit} className="flex flex-col gap-4">
            <div className="relative">
              ...
              <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {wordCount} {t.analyze.wordsLabel} / {charCount} {t.analyze.charsLabel}
                </span>
                <span>
                  {t.analyze.minChars} {MIN_TEXT_LENGTH}, {t.analyze.maxChars}{" "}
            ...
            >
              ...
              {selectedFile ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-foreground">
                      {t.analyze.dropFileHere}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {t.analyze.fileFormats} {MAX_FILE_SIZE_MB}MB
                    </p>
                  </div>
                </div>
              )}
